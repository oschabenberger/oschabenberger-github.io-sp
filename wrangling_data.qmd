...
A great cheat sheet for wrangling data in `R` with `dplyr` and `tidyr` can be found  [here](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf). 
...
The data are arranged very neatly, each year in a separate row, each month in a separate column.
...
In general, the structure of a non-tabular format is not known to `R` and must be wrangled by the user.
Consider the famous Gapminder dataset. The [Gapminder Foundation](https://en.wikipedia.org/wiki/Gapminder_Foundation) is a Swedish non-profit that promotes the United Nations sustainability goals through use of statistics. The Gapminder data set tracks economic and social indicators like life expectancy and the GDP per capita of countries over time. 
