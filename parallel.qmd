...
# Parallel Computing in `R` {sec-parallel-R}
...
the R Studio process is consuming only 3.7\% of available CPU and is running 35 
...
share data, a form of inter-process communication such as a socket must be 
established.
...
creating separate processes that communicate over sockets is the only way in which 
...
If one uses a parallel execution model with forks, or a socket-based execution model
on a single machine, or a clustered execution model with multiple machines, the
overhead is even greater than for the multithreaded model.
...
### Amdahl's Law {#sec-parallel-Amdahl}
...
Because you cannot directly manage thread from the `R` session, parallel programming
...
`foreach` iterates over the variable a and adds its value to the vector b = [10, 10, 10].
...
even operators such as `+` or `*` to combine results. If 
...
load the `doParallel` library and registers a parallel backend with the 
...
the number of machines in `makeCluster'.
...
compared to serial execution. This is not bad! A 4x speedup would mean linear
...
@fig-boot-parallel displays the bootstrap distributions of the logistic
parameter estimates.

```{r, echo=FALSE, fig.align="center", width="80"}
#| fig.cap: Bootstrap distribution of intercept and slopes, $B$=10,000.
#| label: fig-boot-parallel
#| 
par(mfrow=c(1,2))
...
generalizes best to new observations (@fig-cross-parallel).

``` {r cross_val, fig.align="center", width="90%" }
#| fig.cap: Cross-validated prediction sum of squares for 100 regression models.
#| label: fig-cross-parallel
#| 
n_rows <- 500
n_cols <- 100
n_folds <- 10
